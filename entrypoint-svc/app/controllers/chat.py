...
@router.post("/", response_model=NewChat, dependencies=[Depends(require_token)])
async def create(request: Request, chat: NewChat):
    user_auth = request.state.user_auth
    access_token = user_auth['access_token']
    ...
    if chat.username not in usernames or chat.username == user_info['preferred_username']:
