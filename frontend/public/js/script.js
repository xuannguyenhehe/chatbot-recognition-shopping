 ...
 */
 ...
 var Botkit = {
	 ...
	 connectWebsocket: function (ws_url) {
		 ...
		 var connectEvent = 'admin_hello';
		 if (Botkit.getCookie('botkit_guid')) {
			 that.guid = Botkit.getCookie('botkit_guid');
			 connectEvent = 'admin_hello';
		 } else {
			 that.guid = that.generate_guid();
			 Botkit.setCookie('botkit_guid', that.guid, 1);
		 }
	 ...
	 boot: function (user) {
		 ...
		 that.on('sent', function () {
			...
			//  document.getElementById("messenger_input").value = "OKII";
		 });
		 ...
		 that.on('message', function (message) {
			// that.clearReplies();
			if (message.text != "Hệ thống hiện có các khách hàng sau" && message.text != "Xin chào admin") {
				document.getElementById("messenger_input").value = message.text;
		 	}
		 });
